...
class BowlingGameSpec extends Specification implements DomainUnitTest<BowlingGame> {
    ...
    void "test finished sample game 3"() {
        when:
        def game = bowlingRulesService.parse("4 5 X X X X X X X X X 3 7")

        then:
        game.aggregatedScore(0) == 9
        game.aggregatedScore(1) == 39
        game.aggregatedScore(2) == 69
        game.aggregatedScore(3) == 99
        game.aggregatedScore(4) == 129
        game.aggregatedScore(5) == 159
        game.aggregatedScore(6) == 189
        game.aggregatedScore(7) == 219
        game.aggregatedScore(8) == 242
        game.aggregatedScore(9) == 262
    }
